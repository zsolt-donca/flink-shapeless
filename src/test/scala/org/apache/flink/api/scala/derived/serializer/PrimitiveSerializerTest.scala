...
import org.apache.flink.api.scala.derived.serializer.instances.L01_Primitives
...
class PrimitiveSerializerTest extends PropSpec with GeneratorDrivenPropertyChecks with L01_Primitives {
  ...
  private def roundTrip[T <: AnyVal : ClassTag : Serializer : Arbitrary](value: T): Unit = {
    ...
    assert(bytes.size == expectedSizeOf[T])
